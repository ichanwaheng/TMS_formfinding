...
@dataclass
class ConicExample1Settings:
    # Exercise 1 geometry from Gosling et al. (2013), Fig. 3
    base_size: float = 14.0
    ring_diameter: float = 4.0
    ring_height: float = 5.0
    ...
    # Material constants from Exercise 1 (tracked for reporting)
...
def build_conic_mesh(settings: ConicExample1Settings):
    if not HAVE_SCIPY:
...
def run_formfinding(
    ...
    fixed_nodes: np.ndarray,
    center_xy: np.ndarray,
    settings: ConicExample1Settings,
):
    ...
    for it in range(1, settings.max_outer_iterations + 1):
        ...
        print(
            f"[Conic Ex1] iter={it:02d} conv={conv:.3e} | "
...
def build_plot(
    ...
    fixed_nodes: np.ndarray,
    title: str,
    out_html: str = "conic_ex1_formfound.html",
):
    if not HAVE_PLOTLY:
...
def main():
    ...
    print("===== Conic Exercise 1 Input Summary =====")
    ...
    print("\n===== Conic Exercise 1 Final Summary =====")
    if history:
    ...
    build_plot(
        ...
        triangles=triangles,
        fixed_nodes=fixed_nodes,
        title="Conic Example 1 form-finding (asymmetric prestress)",
        out_html="conic_ex1_formfound.html",
    )
